...
export class KizCirculationService {
  ...
  async sync(clientId: string, user: AuthUser) {
    await this.requireClient(clientId);
    ...
    const histories = await this.prisma.shippedKizHistory.findMany({
      where: { clientId, shippedAt: { gte: new Date(Date.now() - 366 * 24 * 60 * 60 * 1000) } },
    ...
    const links = assemblies.length
      ? await this.prisma.fbsOrderRequestLink.findMany({
          where: {
            clientId,
            connectionId: { in: [...new Set(assemblies.map((row) => row.connectionId))] },
            orderId: { in: [...new Set(assemblies.map((row) => row.orderId))] },
          },
        })
      : [];
    ...
    await this.prisma.auditLog.create({
      data: {
        ...
        entityId: clientId,
        payload: {
          scannedShipments: histories.length,
  ...
  async updateItem(itemId: string, dto: UpdateKizCirculationItemDto) {
  ...
  async checkItems(dto: CheckKizCirculationItemsDto) {
