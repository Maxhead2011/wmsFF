...
const FBS_BACKGROUND_REFRESH_INTERVAL_MS = Math.max(
  30_000,
  Number(process.env.FBS_AUTO_SYNC_INTERVAL_MS) || 30_000,
);
...
export class MarketplaceConnectionsService implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit() {
    ...
    // before the first marketplace history refresh starts.
  ...
  private scheduleFbsBackgroundRefresh(
    ...
    this.fbsRefreshTimer = setTimeout(async () => {
      this.fbsRefreshTimer = undefined;
      try {
        await this.refreshAllFbsClients();
      } finally {
        if (!this.fbsBackgroundRefreshStopped) {
          this.scheduleFbsBackgroundRefresh();
        }
      }
        ...
        Array.from({ length: workerCount }, async () => {
          while (queue.length > 0) {
            ...
            try {
              ...
              const refreshed = await this.loadFbsOrders(clientId, previousOrderStates);
  ...
  async repairFbsRequestSelection(requestId: string, user: AuthUser) {
    ...
    // Older requests can contain the full persisted WB composition while only
    // part of it reached the TSD queue. Restore untouched queue rows from the
    // saved request snapshots; existing physical work is protected by the
    // marketplace/order unique key.
    ...
    await this.prisma.clientRequestEvent.create({
      data: {
        ...
        body: repairableTaskIds.length > 0
          ? `Восстановлено недостающих заданий: ${restoredTasks}. Проверено заданий без физических сканов: ${repairableTaskIds.length}. Для выданных сотрудникам заданий сохранены сотрудники и устройства; источники заново выбраны из доступных паллет-сортов.`
          : 'Заданий без физических сканов для восстановления паллет-сортов нет.',
        createdByUserId: user.id,
      },
    });
  ...
  async reconcileFbsStockItem(dto: ReconcileFbsStockItemDto, user: AuthUser) {
    ...
    const targetAmount = stockPlan.quantities.get(sku.id)?.sellable ?? 0;
    const wbAmounts = await this.fetchWildberriesStockAmounts(
    ...
    if (previousAmount <= targetAmount) {
      return { corrected: false, skuId: sku.id, previousAmount, amount: previousAmount, targetAmount, checkedAt: new Date().toISOString() };
    }

    await this.putWildberriesStocks(context.connection.apiKey, context.warehouse.id, [
      { chrtId: ids.chrtId, amount: targetAmount },
    ]);
    const correctedAt = new Date();
    await this.prisma.$transaction([
      this.prisma.fbsStockPublication.updateMany({
        where: { connectionId: context.connection.id, warehouseId: context.warehouse.id, skuId: sku.id },
        data: {
    ...
    return { corrected: true, skuId: sku.id, previousAmount, amount: targetAmount, targetAmount, checkedAt: correctedAt.toISOString() };
  }
  ...
  async scanFbsTsdBox(taskId: string, payload: Record<string, unknown>, user: AuthUser) {
    ...
    if (freeQuantity < task.itemCount) {
      const released = await this.releaseBackgroundFbsReservationsForScannedBox({
        clientId: task.clientId,
    ...
    const updated = await this.updateFbsTsdUnderLease(task, user, {
        ...
        reservedAt: task.reservedAt ?? new Date(),
        boxId: box.id,
        boxCode: box.code,
        errorMessage: null,
    });
    ...
    return {
      ...
      task: {
        ...
        storageBoxes: storageBoxes.map((box) => ({
          ...box,
          location: locationsByBox.get(box.code) ?? null,
        })),
        nextRequestSources,
        recommendedLocation,
  ...
  /**
   * A real request which is already being picked has priority over virtual
   * background reservations. If the picker scans a physically valid box, we
   * release only untouched AUTO reservations occupying the required units.
   * The background synchronizer will immediately route those orders to other
   * free boxes; physically started tasks are never moved.
   */
  private async releaseBackgroundFbsReservationsForScannedBox(input: {
    clientId: string;
    requestId: string;
    taskId: string | null;
    skuId: string;
  ...
  private async fbsTsdNextRequestSources(task: FbsTsdAssemblyRecord) {
    ...
    return tasks.flatMap((candidate) => {
      const stockSkuId = candidate.sourceSkuId ?? candidate.skuId;
      const reservedByBox = new Map<string, number>();
      (reservationRowsBySku.get(stockSkuId) ?? []).forEach((reservation) => {
        if (!reservation.boxId) return;
        reservedByBox.set(
          reservation.boxId,
          (reservedByBox.get(reservation.boxId) ?? 0) + reservation.itemCount,
      ...
      const boxes = (boxesBySku.get(stockSkuId) ?? [])
        .map((box) => ({
          ...
          freeQuantity:
            box.quantity -
            (reservedByBox.get(box.id) ?? 0) +
            (candidateAssignedBoxId === box.id ? Math.max(1, candidate.itemCount) : 0),
        }))
        .filter((box) => box.freeQuantity >= candidate.itemCount)
        .sort(
          (left, right) =>
  ...
  private async formatFbsTsdPalletScan(
    ...
    const nearbyPallets = zoneRoutes
      .filter((route) => route.palletId !== pallet.id && route.boxCodes.length > 0)
      .map((route) => ({
        id: route.palletId,
        code: route.palletCode,
        neededBoxes: route.boxCodes.length,
        neededBoxCodes: route.boxCodes,
      }));
    const message =
      neededBoxCodes.length > 0
        ? `Паллетсорт ${pallet.code}: для заявки нужен ${neededBoxCodes.length} короб(а/ов). Берите короба из показанного списка.${nearbyPallets.length > 0 ? ` После него в этой же зоне есть ещё ${nearbyPallets.length} паллетсорт(а/ов) для этой заявки.` : ''}`
    ...
    return uniqueStrings(
      placements.flatMap((placement) => {
        ...
        const needed = stockSkuIds.some((stockSkuId) => {
          ...
          const reservations = reservationRowsBySku.get(stockSkuId) ?? [];
          const reserved = reservations
            .filter((reservation) => reservation.boxId === placement.boxId)
            .reduce((sum, reservation) => sum + reservation.itemCount, 0);
          const releasableBackground = reservations
            .filter(
              (reservation) =>
                reservation.boxId === placement.boxId &&
          ...
          return (candidatesBySku.get(stockSkuId) ?? []).some((candidate) => {
            ...
            const ownReservation = assignedBoxId === placement.boxId
              ? Math.max(1, candidate.itemCount)
              : 0;
            const freeIfScanned =
              available - reserved + ownReservation + releasableBackground;
            return freeIfScanned >= Math.max(1, candidate.itemCount);
          });
        });
  ...
  private async fbsTsdReservationRows(input: {
    ...
    const completedRequestIds = uniqueStrings(
      tasks
    ...
    const [openCompletedRequests, obsoleteCompletedOrderLinks, latestPhysicalRecount] = await Promise.all([
      completedRequestIds.length > 0
        ? this.prisma.clientRequest.findMany({
            where: {
              id: { in: completedRequestIds },
              status: { notIn: [...FBS_REQUEST_CLOSED_STATUSES] },
            },
            select: { id: true },
          })
        : [],
      completedRequestIds.length > 0 && canCheckCompletedOrderLinks
        ? this.prisma.fbsOrderRequestLink.findMany({
            where: {
              requestId: { in: completedRequestIds },
        ...
        : [],
      input.boxId && completedRequestIds.length > 0
        ? this.prisma.stockMovement.findFirst({
            where: {
              clientId: input.clientId,
              skuId: input.skuId,
    ...
    const openCompletedRequestIds = new Set(
      openCompletedRequests.map((request) => request.id),
    );
    ...
    return tasks
      .filter(
        (task) =>
          ...
          (task.status !== 'COMPLETED' ||
            (openCompletedRequestIds.has(task.requestId) &&
  ...
  private async fbsTsdReservationRowsBySku(input: {
    ...
    const [openCompletedRequests, obsoleteCompletedOrderLinks] = await Promise.all([
      completedRequestIds.length > 0
        ? this.prisma.clientRequest.findMany({
            where: {
              id: { in: completedRequestIds },
  ...
  async moveFbsOrdersToNewSupply(dto: FbsOrderSelectionDto, user: AuthUser) {
    ...
    const skippedOrders: Array<{ id: string; reason: string }> = handledTasks.map(
      (task) => {
    ...
    try {
      ...
      // WB can return the old source supply for a few seconds after a
      // successful PATCH. The transaction above has already recorded the
      // target supply atomically. Do not immediately resync the synthetic
      // response here: it could overwrite the new link/task with that stale
      // source supply. The normal next refresh syncs only the confirmed WB
      // state and keeps the MOVING link intact until then.
  ...
  private async resolveFbsWarehouseFromWildberries(
    ...
    for (const connectionId of connectionIds) {
      ...
      try {
        const [sellerWarehouses, offices] = await Promise.all([
          this.fetchWildberriesStockWarehouses(connection.apiKey),
          this.fetchWildberriesOffices(connection.apiKey),
        ]);
        ...
        for (const order of unresolvedOrders) {
          const sellerWarehouse = sellerWarehouses.find(
            (warehouse) => warehouse.id === order.warehouseId,
          );
          const officeId = order.officeId || sellerWarehouse?.officeId || '';
          ...
          const city =
            ...
            sellerWarehouse?.name ||
            '';
          const cityKey = normalizeFbsCity(city);
          if (!cityKey) continue;
          const branch = branches.find((candidate) => {
            const candidateCity = normalizeFbsCity(candidate.city);
            const candidateName = normalizeFbsCity(candidate.name);
            return (
              candidateCity === cityKey ||
              cityKey.includes(candidateCity) ||
              candidateName.includes(cityKey)
            );
          });
          if (branch) {
            matchedBranches.set(branch.id, {
        ...
        this.logger.warn(
          `WB warehouse lookup failed for connection ${connectionId}: ${message}`,
        );
      }
    }
...
const WB_REQUEST_INTERVAL_MS = 240;
const MARKETPLACE_RATE_LIMIT_RETRIES = 4;
const wbNextRequestAt = new Map<string, number>();

async function waitForWbRequestSlot(url: string, init: RequestInit) {
  if (!url.includes('wildberries.ru')) return;
  const headers = new Headers(init.headers);
  const token = headers.get('authorization')?.trim();
  if (!token) return;
  const now = Date.now();
  const scheduledAt = Math.max(now, wbNextRequestAt.get(token) ?? now);
  wbNextRequestAt.set(token, scheduledAt + WB_REQUEST_INTERVAL_MS);
  if (scheduledAt > now) {
    await delayWithAbort(scheduledAt - now, init.signal);
  }
}
...
async function marketplaceFetch(
  ...
  for (let attempt = 0; attempt <= rateLimitRetries; attempt += 1) {
    ...
    await delayWithAbort(retryDelayMs, init.signal);
  }
