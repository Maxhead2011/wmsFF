...
export type AnalysisStock = { skuId: string; name: string; barcode: string; available: number; saleLimit?: number | null; enabled?: boolean };

// FIX: a deterministic, read-only recommendation; it never changes saved shares.
export function analyzeWbStockDemand(orders: Order[], connectionId: string, stocks: AnalysisStock[], shares: FbsStockAllocationShare[], reserve: WbStockReserve, now = Date.now(), days = 30, threshold = 10) {
  ...
  const rows = stocks.map(stock => {
    ...
    const mean = (demand?.total ?? 0) / days;
    const cv = mean > 0 ? Math.sqrt(demand!.daily.reduce((sum, x) => sum + (x - mean) ** 2, 0) / days) / mean : null;
    const activeDays = demand?.daily.filter(x => x > 0).length ?? 0;
    const sufficient = (demand?.total ?? 0) >= 10 && activeDays >= 7;
    ...
    const suggested = sufficient ? recommendFbsStockPercentages(shares.map(s => s.warehouseId), demand!.demand) : shares;
    ...
    const afterReserve = wbStockAfterReserve(stock.available, reserve);
    const publishable = stock.enabled === false ? 0 : Math.min(afterReserve, stock.saleLimit == null ? afterReserve : Math.max(0, stock.saleLimit));
    ...
    return { ...stock, reserveQuantity: Math.max(0, stock.available - afterReserve), publishable, orderedUnits: demand?.total ?? 0, activeDays, abc: abc.get(stock.skuId) ?? '—', xyz, coefficientOfVariation: cv, sufficient, percentages, allocation };
  });
  const hasEvidence = rows.some(r => r.sufficient && r.publishable > 0);
  const recommendedShares = hasEvidence && [...totals.values()].some(n => n > 0) ? recommendFbsStockPercentages(shares.map(s => s.warehouseId), totals) : shares.map(s => ({ warehouseId: s.warehouseId, percent: s.percent }));
  return { periodDays: days, generatedAt: new Date(now).toISOString(), from: new Date(start).toISOString(), orderedUnits: totalUnits, excluded, hasEvidence, recommendedShares, rows,
    warnings: ['ABC рассчитан по числу заказанных единиц (80/95%), не по выручке.', 'XYZ: коэффициент вариации дневного спроса; X ≤ 0,5; Y ≤ 1; Z > 1. Для оценки нужно ≥ 10 единиц и ≥ 7 дней с заказами.', 'История наличия по складам WB не учитывается. Нулевой спрос может быть связан с отсутствием товара. Рекомендация требует проверки.'] };
}
