...
describe('SKU label lookup', () => {
  it('searches the article within the selected client scope', async () => {
    ...
    expect(findMany).toHaveBeenCalledWith(expect.objectContaining({ where: expect.objectContaining({
      clientId: 'client-1', OR: expect.arrayContaining([{ article: { contains: 'спорт_синий', mode: 'insensitive' } }]),
    }) }));
  });
});
