...
export const INTERNAL_API_DEFINITIONS: readonly InternalApiDefinition[] = Object.freeze([
  ...
  },
  {
    ...
    prefixes: ['/marketplace-connections', '/marketplace-connection', '/external/v1/fbs',
      ...
      '/marketplace-connections/allocation', '/marketplace-connections/duplicate-groups', '/marketplace-connections/stock-confirmation'],
    // FIX: preserve delivery-options plus six reshipment handlers; aliases count once.
    routeCount: 123, // FIX: duplicate group preview plus reviewed client application.
    description: 'Подключения WB/Ozon, заказы FBS, поставки, статусы, финансовые отчёты и распределение остатков. Подготовка WB/Ozon: проверка кабинетов, черновик долей и предпросмотр без отправки. Группы дублей: соответствия из переклейки, доли по размерам, страховой резерв и предпросмотр без отправки.',
