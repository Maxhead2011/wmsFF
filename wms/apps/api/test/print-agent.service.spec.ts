...
function fixture() {
  const prisma = {
    ...
    printJob: {
      ...
      findFirst: vi.fn().mockResolvedValue({ id: 'job-1', status: 'queued', processedAt: null, payload: { imageBase64: png, copies: 2, widthMm: 40, heightMm: 60 } }),
...
describe('PrintAgentService', () => {
  it('queues a scoped 40 × 60 image for the selected station and copy count', async () => {
    const { service, prisma } = fixture();
    const job = await service.createSkuJob({ stationId: 'station-1', skuId: 'sku-1', barcode: '2041234567890', imageBase64: png, copies: 2, widthMm: 40, heightMm: 60 }, { id: 'user-1' } as never);
  ...
  it('rejects a barcode absent from the card before queueing', async () => {
    const { service, prisma } = fixture();
    await expect(service.createSkuJob({ stationId: 'station-1', skuId: 'sku-1', barcode: 'other', imageBase64: png, widthMm: 40, heightMm: 60 }, { id: 'user-1' } as never)).rejects.toThrow('не найден');
