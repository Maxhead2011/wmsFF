...
it('writes off confirmed missing balances and retires active marks without changing SHIPPING history', async () => {
  ...
  // TEST: administrator-confirmed empty sources cannot retain active PACKING/RESERVED composition.
  expect(tx.productMark.updateMany).toHaveBeenCalledWith({ where: { boxId: 'a', clientId: 'client', status: { in: ['AVAILABLE','PACKING','RESERVED'] } }, data: { status: 'BLOCKED' } });
