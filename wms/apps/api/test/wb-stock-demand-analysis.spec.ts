...
const availability = ['a', 'b'].flatMap(warehouseId => Array.from({ length: 30 }, (_, i) => ({ skuId: 's', warehouseId, day: new Date(now - (i+1) * 86400000).toISOString().slice(0,10), observedMask: 0xffffff, positiveMask: 0xffffff })));
...
describe('WB demand analysis', () => {
  ...
  it('isolates account, dates, cancellation, warehouse and duplicate order IDs', () => {
    ...
    const result = analyzeWbStockDemand([...orders, orders[0], order(20, { category: 'cancelled' }), order(21, { connectionId: 'other' }), order(22, { createdAt: null }), order(23, { warehouseId: 'excluded' }), order(24, { createdAt: new Date(now + 1000).toISOString() })], 'c', stocks, shares, { mode: 'NONE', value: 0 }, now, 30, 10, { availability });
