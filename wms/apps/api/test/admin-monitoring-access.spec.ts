...
describe('ADMIN monitoring read access', () => {
  ...
  it('does not grant owner actions or administration with monitoring read access', async () => {
    // TEST: monitoring controls do not authorize app updates or general owner tools.
    vi.stubEnv('ADMIN_MONITORING_ENABLED', 'true');
    const { service } = setup();
    await expect(service.issueTsdMonitorAction('TSD-1', 'UPDATE_APP', admin as never)).rejects.toThrow(ForbiddenException);
