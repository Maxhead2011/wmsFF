...
describe('AdministrationInternalApiService', () => {
  ...
  it('регистрирует переотгрузку WB без дублирования алиаса и объясняет подтверждённую запись', () => {
    ...
    // TEST: the three size-substitution endpoints are documented with the FBS group.
    expect(definition.prefixes).toContain('/marketplace-connections/fbs/size-substitution');
    expect(definition.routeCount).toBe(110);
