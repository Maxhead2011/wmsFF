...
// TEST: optional native V8 coverage for the changed method, without extra packages.
const coverageFile = process.env.WMS_SOS_COVERAGE_REPORT;
const profiler = new Session();
const post = (method: string, params = {}) => new Promise<any>((resolve, reject) =>
  profiler.post(method as any, params, (error, result) => error ? reject(error) : resolve(result)));
beforeAll(async () => {
  if (!coverageFile) return;
  profiler.connect(); await post('Profiler.enable');
...
afterAll(async () => {
  ...
  try {
    ...
    const script = coverage.result.find((s: any) => s.url.includes('marketplace-connections.service.ts') &&
      s.functions.some((f: any) => f.functionName === 'listSosWbRequests'));
    expect(script).toBeDefined();
    const method = script.functions.find((f: any) => f.functionName === 'listSosWbRequests');
    const outer = method.ranges[0];
    const ranges = script.functions.flatMap((f: any) => f.ranges).filter((r: any) =>
      r.startOffset >= outer.startOffset && r.endOffset <= outer.endOffset);
    const covered = ranges.filter((r: any) => r.count > 0).length;
    writeFileSync(coverageFile, JSON.stringify({ method: 'listSosWbRequests', coveredRanges: covered,
      totalRanges: ranges.length, percent: 100 * covered / ranges.length }));
    expect(covered / ranges.length).toBeGreaterThanOrEqual(0.8);
