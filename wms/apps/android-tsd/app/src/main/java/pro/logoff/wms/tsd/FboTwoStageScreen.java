...
final class FboTwoStageScreen {
    ...
    FboTwoStageScreen(Activity activity,TsdSession session,WmsApi api,String baseUrl,String id,Runnable back) {
    ...
    private void render(){
        ...
        text(root,"Сборка ФБО");if(!message.isEmpty())text(root,message);
        input=null;
        if(plan!=null){text(root,plan.title);text(root,"Нужно "+plan.needed+" · Отобрано "+plan.picked+" · Упаковано "+plan.packed);
            ...
            if("NOT_STARTED".equals(plan.phase))button(root,"Начать отбор",ready(),()->send("START",null));
                ...
                if("PICKING".equals(plan.phase)){
                    ...
                    if(r!=null){text(root,r.boxCode+" · "+r.pallet+" · "+r.zone);for(TsdFboPlan.Task t:r.tasks)text(root,"Отберите "+t.quantity+" ед. · "+t.name+" · "+t.barcode);
                        ...
                        for(TsdFboPlan.Route row:plan.route)if(row.pallet.equals(state.pallet)){text(root,(row.pallet.isEmpty()?"Без паллета":row.pallet)+" · "+row.zone+" → "+row.boxCode);for(TsdFboPlan.Task t:row.tasks)text(root,t.name+": "+t.quantity+" ед.");}
                    }
                    ...
                    button(root,"Перейти к упаковке",ready()&&plan.picked==plan.needed,()->send("FINISH_PICK",null));
    ...
    void submit(){handler.removeCallbacks(automatic);if(!ready()||input==null||plan==null)return;String value=input.getText().toString().trim();if(value.isEmpty())return;input.setText("");message="";
        ...
        if("PICKING".equals(plan.phase)&&state.source.isEmpty()){if(!state.scanLocation(plan,value))message=state.pallet.isEmpty()?"Сначала отсканируйте паллет. Короб без паллета можно сканировать сразу.":"Короб не входит в список на этом паллете.";render();return;}
        if("PACKING".equals(plan.phase)&&state.target.isEmpty()){if(plan.wholeBoxes.contains(value)){state.source=value;send("PACK_BOX",null);}else{state.target=value;send("OPEN_BOX",null);}return;}
