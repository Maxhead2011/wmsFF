...
export function FactoryPanel({ session }: { session: AuthSession }) {
  ...
  // ADDED: клиент работает со своей фабрикой, но не получает управление пользователями и настройками клиентов.
  const canManageFactorySetup = session.user.permissionCodes.includes('system:admin') || (
    session.user.permissionCodes.includes('users:write') &&
  ...
  async function loadSetup() {
    // FIX: клиентская роль не запрашивает закрытый для неё список пользователей.
    const [nextClients, nextUsers] = await Promise.all([
      fetchClients(session.accessToken),
      canManageFactorySetup ? fetchUsers(session.accessToken) : Promise.resolve([]),
    ]);
  ...
  return <div className="factory-panel">
    ...
    {canManageFactorySetup ? <section className="factory-setup" aria-label="Подключение фабрики">
    ...
    {!canManageFactorySetup && selectedClient && !selectedClient.factoryEnabled ? (
      <p className="factory-message">Фабрика для вашего кабинета ещё не подключена. Обратитесь к менеджеру WMS.</p>
    ) : null}
