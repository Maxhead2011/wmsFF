...
export function FbsPanel({ session, onOpenRequest }: FbsPanelProps) {
  ...
  const [supplyRequestFeedback, setSupplyRequestFeedback] = useState<{
    tone: 'success' | 'error';
    text: string;
  } | null>(null);
  ...
  const [supplyReconcilePreview, setSupplyReconcilePreview] =
    useState<FbsSupplyReconciliation | null>(null);
  const [supplyReconcileError, setSupplyReconcileError] = useState('');
  ...
  useEffect(() => {
    ...
    setSupplyReconcilePreview(null);
    setSupplyReconcileError('');
    setSupplyReconcileBusy(false);
    supplyReconcileRequestSequence.current += 1;
  ...
  async function submitSupplyRequest(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const supplyId = supplyRequestId.trim().toUpperCase();
    if (!selectedClientId || !supplyId || supplyRequestBusy) return;
    setSupplyRequestBusy(true);
    setSupplyRequestFeedback(null);
    setOrderActionMessage('');
    setOrderActionError('');
    try {
      const result = await createFbsRequestFromSupply(session.accessToken, {
        clientId: selectedClientId,
        supplyId,
      });
      setSelectedOrderKeys(new Set());
      setSupplyRequestId(result.supplyId);
      setSupplyRequestFeedback({
        tone: 'success',
        text:
          `Создана заявка WMS №${String(result.request.number).padStart(6, '0')}: ` +
          `${result.linkedOrders} заказ(а/ов) из поставки ${result.supplyId}.` +
          (result.skippedLinkedOrders > 0
            ? ` Уже были в заявках: ${result.skippedLinkedOrders}.`
            : '') +
          (result.skippedInactiveOrders > 0
            ? ` Неактивных пропущено: ${result.skippedInactiveOrders}.`
            : ''),
      });
      ...
      setSupplyRequestFeedback({
        tone: 'error',
        text: caught instanceof Error ? caught.message : 'Не удалось создать заявку из поставки WB.',
      });
    } finally {
  ...
  async function submitSupplyReconciliation(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const supplyId = supplyReconcileId.trim().toUpperCase();
    if (!selectedClientId || !supplyId || supplyReconcileBusy) return;
    const wbConnections = ordersState.data?.connections.filter(
      (connection) => connection.marketplace === 'WILDBERRIES',
    ) ?? [];
    const connectionId =
      ordersState.data?.orders.find(
        (order) => order.marketplace === 'WILDBERRIES' && order.supplyId === supplyId,
      )?.connectionId ??
      (wbConnections.length === 1 ? wbConnections[0]?.id : undefined);
    if (!connectionId) {
      setSupplyReconcileError(
        'Не удалось определить кабинет WB этой поставки. Обновите заказы FBS и повторите проверку.',
      );
      return;
    }
    setSupplyReconcileBusy(true);
    setSupplyReconcileError('');
    setSupplyReconcilePreview(null);
    const requestSequence = ++supplyReconcileRequestSequence.current;
    try {
      const preview = await previewFbsSupplyReconciliation(session.accessToken, {
        clientId: selectedClientId,
        connectionId,
        supplyId,
      });
      // FIX: ignore a late response after client/marketplace was switched.
      if (requestSequence !== supplyReconcileRequestSequence.current) return;
      setSupplyReconcileId(preview.supplyId);
      setSupplyReconcilePreview(preview);
    } catch (caught) {
      if (requestSequence !== supplyReconcileRequestSequence.current) return;
      setSupplyReconcileError(
        caught instanceof Error ? caught.message : 'Не удалось сверить поставку с Wildberries.',
      );
    } finally {
      if (requestSequence === supplyReconcileRequestSequence.current) {
        setSupplyReconcileBusy(false);
      }
    }
  }

  async function applySupplyReconciliation() {
    const preview = supplyReconcilePreview;
    if (!preview?.canApply || preview.applied || supplyReconcileBusy) return;
    const requestNumbers = preview.requests
      .map((request) => `№${String(request.number).padStart(6, '0')}`)
      .join(', ');
    const confirmed = window.confirm(
      `Исправить состав поставки ${preview.supplyId} по данным Wildberries?\n\n` +
        `WB: ${preview.wbOrderCount} заказов. WMS сейчас: ${preview.localTargetOrderCount}.\n` +
        `Связи заказов: ${preview.linkUpdates}, задания ТСД: ${preview.taskUpdates}.` +
        (requestNumbers ? `\nЗаявки WMS: ${requestNumbers}.` : '') +
        '\n\nОстатки, резервы, короба и КИЗы изменяться не будут.',
    );
    if (!confirmed) return;
    setSupplyReconcileBusy(true);
    setSupplyReconcileError('');
    const requestSequence = ++supplyReconcileRequestSequence.current;
    try {
      const result = await applyFbsSupplyReconciliation(session.accessToken, {
        clientId: preview.clientId,
        connectionId: preview.connectionId,
        supplyId: preview.supplyId,
        fingerprint: preview.fingerprint,
      });
      // FIX: never restore client A's result after the operator switched to B.
      if (requestSequence !== supplyReconcileRequestSequence.current) return;
      setSupplyReconcilePreview(result);
      await loadOrders(true);
    } catch (caught) {
      if (requestSequence !== supplyReconcileRequestSequence.current) return;
      setSupplyReconcileError(
        caught instanceof Error ? caught.message : 'Не удалось исправить состав поставки.',
      );
    } finally {
      if (requestSequence === supplyReconcileRequestSequence.current) {
        setSupplyReconcileBusy(false);
    ...
    try {
      for (const group of groups) {
        ...
        try {
          ...
          setOrderActionMessage(
            `Создаю заявки: ${created} из ${groups.length}. Привязано заказов: ${linked} из ${orderCount}.`,
          );
        } catch (caught) {
          failures.push(
      ...
      <section className="fbs-workspace" role="tabpanel" aria-label={activeConfig.title}>
        ...
        {marketplace === 'WILDBERRIES' && activeView === 'active' && selectedClientId ? (
          <form className="fbs-supply-request-tool" onSubmit={submitSupplyRequest}>
            <span className="fbs-supply-request-tool__icon">
              <FilePlus2 size={20} aria-hidden="true" />
            </span>
            <label>
              <span>Создать заявку WMS из поставки WB</span>
              <input
                value={supplyRequestId}
                onChange={(event) => {
                  setSupplyRequestId(event.target.value.toUpperCase());
                  setSupplyRequestFeedback(null);
                }}
                placeholder="Например, WB-GI-267374795"
                aria-label="Номер поставки Wildberries"
                autoComplete="off"
              />
            </label>
            <button
              ...
              type="submit"
              disabled={!supplyRequestId.trim() || supplyRequestBusy}
            >
              {supplyRequestBusy ? 'Проверяю WB…' : 'Найти и создать заявку'}
            </button>
            <small>
              WMS возьмёт активные заказы этой поставки. Уже привязанные заказы не дублируются; сама поставка WB не изменяется.
            </small>
            {supplyRequestFeedback ? (
              <p className={`fbs-supply-request-tool__feedback is-${supplyRequestFeedback.tone}`}>
                {supplyRequestFeedback.text}
              </p>
            ) : null}
          </form>
        ) : null}
        ...
        {marketplace === 'WILDBERRIES' && activeView === 'active' && selectedClientId ? (
          <form className="fbs-supply-request-tool" onSubmit={submitSupplyReconciliation}>
            <span className="fbs-supply-request-tool__icon">
              <ArrowRightLeft size={20} aria-hidden="true" />
            </span>
            <label>
              <span>Синхронизировать состав поставки с WB</span>
              <input
                value={supplyReconcileId}
                onChange={(event) => {
                  setSupplyReconcileId(event.target.value.toUpperCase());
                  setSupplyReconcilePreview(null);
                  setSupplyReconcileError('');
                }}
                placeholder="Например, WB-GI-269013701"
                aria-label="Поставка WB для синхронизации состава"
                autoComplete="off"
              />
            </label>
            <button
              className="button button-primary"
              type="submit"
              disabled={!supplyReconcileId.trim() || supplyReconcileBusy}
            >
              {supplyReconcileBusy ? 'Проверяю…' : 'Проверить состав'}
            </button>
            <small>
              Для поставок, объединённых вручную в кабинете WB. Сначала WMS покажет план изменений и только после подтверждения обновит номера поставок.
            </small>
            {supplyReconcileError ? (
              <p className="fbs-supply-request-tool__feedback is-error">
                {supplyReconcileError}
              </p>
            ) : null}
            {supplyReconcilePreview ? (
              <div
                className={`fbs-supply-request-tool__feedback ${
                  supplyReconcilePreview.blockers.length > 0 ? 'is-error' : 'is-success'
                }`}
              >
                <strong>
                  {supplyReconcilePreview.applied
                    ? supplyReconcilePreview.message
                    : supplyReconcilePreview.changed
                      ? `Найдено расхождение: WB — ${supplyReconcilePreview.wbOrderCount}, WMS — ${supplyReconcilePreview.localTargetOrderCount}.`
                      : `Расхождений нет: ${supplyReconcilePreview.wbOrderCount} заказов.`}
                </strong>
                <br />
                <span>
                  Заявок WMS: {supplyReconcilePreview.requests.length}; связей к обновлению: {supplyReconcilePreview.linkUpdates}; заданий ТСД: {supplyReconcilePreview.taskUpdates}.
                </span>
                {supplyReconcilePreview.plans.some((plan) => !plan.target && plan.movedToTarget > 0) ? (
                  <>
                    <br />
                    <span>
                      Старые поставки: {supplyReconcilePreview.plans
                        .filter((plan) => !plan.target && plan.movedToTarget > 0)
                        .map((plan) => `${plan.supplyId} (${plan.movedToTarget})`)
                        .join(', ')}.
                    </span>
                  </>
                ) : null}
                {supplyReconcilePreview.blockers.map((blocker) => (
                  <span key={blocker}>
                    <br />{blocker}
                  </span>
                ))}
                {supplyReconcilePreview.canApply && !supplyReconcilePreview.applied ? (
                  <>
                    <br />
                    <button
                      className="button button-primary"
                      type="button"
                      disabled={supplyReconcileBusy}
                      onClick={() => void applySupplyReconciliation()}
                    >
                      Исправить состав в WMS
                    </button>
                  </>
                ) : null}
              </div>
