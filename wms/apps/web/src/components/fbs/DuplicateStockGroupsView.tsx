...
import { fetchDuplicateGroups, fetchDuplicateCatalog, saveDuplicateGroup, previewDuplicateGroup, type AuthSession } from '../../lib/api';
...
export function DuplicateStockGroupsView({ session, clientId }: { session: AuthSession; clientId: string }) {
  ...
  useEffect(() => {
    const id = ++generation.current; setData(null); setGroup(null); setPreview(null); setCards([]); setBusy(true); setError(''); setMessage('');
  ...
  return <section className="fbs-allocation">
    ...
    {!data ? <p>{busy ? 'Загрузка групп…' : 'Не удалось загрузить группы.'}</p> : <>
      ...
      <fieldset disabled={busy || !data.relabelingEnabled}><legend>Соответствия из «Переклейки»</legend>
        ...
        {group && <button disabled={!selectedMapping || group.shares.length >= 6} onClick={() => selectedMapping && void run(() => addMapping(selectedMapping, true))}>Добавить ещё дубль</button>}
        {!data.mappings.length && <p>Сначала добавьте соответствия артикулов в меню «Переклейка».</p>}
      </fieldset>
    </>}
    {group && <fieldset disabled={busy}><legend>Правило распределения</legend>
      <label>Название группы <input value={group.name} onChange={e => change({ ...group, name: e.target.value })} /></label>
      <label>Кабинет WB <select value={group.connectionId} onChange={e => change({ ...group, connectionId: e.target.value })}><option value="">Выберите кабинет</option>
      ...
      {group.variants.map(v => {
        ...
        return <fieldset key={v.sourceSkuId}><legend>{label(source)}</legend>
          {v.targets.filter(t => t.targetId !== v.sourceSkuId).map(t => {
            ...
            return <div key={t.targetKey}><label>Целевая карточка: {slot.label} <select value={t.targetId} onChange={e => change({ ...group, variants: group.variants.map(x => x.sourceSkuId === v.sourceSkuId ? { ...x, targets: x.targets.map(y => y.targetKey === t.targetKey ? { ...y, targetId: e.target.value, confirmed: false } : y) } : x) })}>
              <option value="">Укажите соответствие</option>{options.map(c => <option value={c.id} key={c.id}>{label(c)}</option>)}</select></label>
              <label><input type="checkbox" checked={t.confirmed} disabled={!selected} onChange={e => change({ ...group, variants: group.variants.map(x => x.sourceSkuId === v.sourceSkuId ? { ...x, targets: x.targets.map(y => y.targetKey === t.targetKey ? { ...y, confirmed: e.target.checked } : y) } : x) })} /> Подтверждаю соответствие и переклейку на {selected?.barcodes[0]?.value || 'целевой ШК'}</label>
            </div>;
          })}
    ...
    {preview && <section><h4>Предпросмотр на {new Date(preview.generatedAt).toLocaleString('ru-RU')}</h4>
      ...
      {preview.publications.length > 0 && <p role="alert">Есть действующие настройки публикации WB. Перед включением процентных правил потребуется согласовать их с новым распределением.</p>}
