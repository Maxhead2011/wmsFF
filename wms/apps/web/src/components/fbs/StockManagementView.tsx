...
import { fetchMarketplaceAllocationCapabilities, fetchMarketplaceAllocation, fetchMarketplaceConnections, type AuthSession } from '../../lib/api';
...
export function StockManagementView({ session, clientId, connectionId, renderStocks }: {
  ...
  useEffect(() => {
    let active = true;
    setSection(''); setEnabled(null); setError('');
    void fetchMarketplaceAllocationCapabilities(session.accessToken).then(async r => {
      if (!active) return;
      if (r.enabled) {
        const settings = await fetchMarketplaceAllocation(session.accessToken, clientId);
        if (!active) return;
  ...
  return <section>
    ...
    {section ? <button type="button" onClick={() => setSection('')}>← Все разделы</button> : <div className="fbs-allocation" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 12 }}>
      ...
      <button type="button" disabled>Между артикулами<br /><small>Следующий этап: соответствия и размеры</small></button>
