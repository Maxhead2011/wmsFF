...
export function KizCirculationPanel({ session }: Props) {
  ...
  // ADDED: массовые действия берут только КИЗы, которые ещё можно проверять или включать в пакет.
  const selectableItems = filteredItems.filter((item) => !['IN_BATCH', 'SUBMITTED', 'APPLIED'].includes(item.status));
  const selectedItems = selectableItems.filter((item) => selectedIds.includes(item.id));
  const allItemsSelected = selectableItems.length > 0 && selectedItems.length === selectableItems.length;
  ...
  async function sync() {
    if (!syncPeriod.periodFrom || !syncPeriod.periodTo) {
      setMessage('');
      setError('Укажите дату начала и окончания выгрузки Wildberries.');
      return;
    }
    if (syncPeriod.periodFrom > syncPeriod.periodTo) {
      setMessage('');
      setError('Дата начала выгрузки не может быть позже даты окончания.');
      return;
    }
    await run('sync', async () => {
      ...
      return `Wildberries за ${formatDateOnly(syncPeriod.periodFrom)}–${formatDateOnly(syncPeriod.periodTo)}: проверено отгрузок ${result.scannedShipments}, новых КИЗ к погашению ${result.retireCreated}, к возврату ${result.returnCreated}.${result.invalidCodes ? ` Не разобрано кодов: ${result.invalidCodes}.` : ''}`;
    });
  }

  async function checkSelected() {
    // FIX: если оператор ничего не отметил, кнопка проверяет всю доступную очередь.
    const itemIds = (selectedItems.length ? selectedItems : selectableItems).map((item) => item.id);
    if (!itemIds.length) return;
    await run('check', async () => {
      const result = await checkKizCirculationItems(session.accessToken, clientId, itemIds);
      setSelectedIds([]);
  ...
  function toggleAllItems() {
    // ADDED: один явный выбор всей текущей очереди с учётом операции и фильтра сервиса.
    setSelectedIds(allItemsSelected ? [] : selectableItems.map((item) => item.id));
  }
  ...
  return (
    <section className="kizc-panel" aria-label="Погашение КИЗ">
      ...
      <nav className="kizc-tabs" aria-label="Разделы погашения КИЗ">
        ...
        <button type="button" className={view === 'settings' ? 'active' : ''} onClick={() => setView('settings')}>API Честного знака</button>
      </nav>

      {view === 'queue' ? (
        <>
          <section className="kizc-sync-panel" aria-label="Период выгрузки продаж Wildberries">
            <div className="kizc-sync-panel__title">
              <CalendarRange size={21} aria-hidden="true" />
              <div>
                <h3>Выгрузка продаж Wildberries</h3>
                <p>Выберите календарный период. WMS возьмёт только фактические отгрузки WB и повторно не создаст уже найденные КИЗ.</p>
              </div>
            </div>
            <div className="kizc-sync-period">
              <label>
                <span>С даты</span>
                <input
                  type="date"
                  value={syncPeriod.periodFrom}
                  max={syncPeriod.periodTo || today()}
                  onChange={(event) => setSyncPeriod((current) => ({ ...current, periodFrom: event.target.value }))}
                />
              </label>
              <label>
                <span>По дату</span>
                <input
                  type="date"
                  value={syncPeriod.periodTo}
                  min={syncPeriod.periodFrom}
                  max={today()}
                  onChange={(event) => setSyncPeriod((current) => ({ ...current, periodTo: event.target.value }))}
                />
              </label>
              <button className="kizc-primary" type="button" disabled={!clientId || Boolean(busy)} onClick={() => void sync()}>
                ...
                {busy === 'sync' ? 'Получаю продажи…' : 'Получить продажи WB'}
              </button>
            </div>
          </section>
          ...
          <section className="kizc-workbar">
            ...
            <label className="kizc-select-all">
              <input type="checkbox" checked={allItemsSelected} disabled={!selectableItems.length || Boolean(busy)} onChange={toggleAllItems} />
              <span>{allItemsSelected ? 'Выбраны все' : `Выбрать все (${selectableItems.length})`}</span>
            </label>
            <span className="kizc-selection">Выбрано: <strong>{selectedItems.length}</strong></span>
            <button type="button" className="kizc-secondary" disabled={!selectableItems.length || Boolean(busy)} onClick={() => void checkSelected()}>
              <ShieldCheck size={17} /> {selectedItems.length ? `Проверить в ЧЗ (${selectedItems.length})` : `Проверить все в ЧЗ (${selectableItems.length})`}
            </button>
          </section>
              ...
              <table className="kizc-table">
                ...
                <tbody>
                  {filteredItems.map((item) => {
                    ...
                    return (
                      <tr key={item.id}>
                        <td><input type="checkbox" aria-label={`Выбрать КИ ${shortCis(item.cis)}`} disabled={!selectable || Boolean(busy)} checked={selectedIds.includes(item.id)} onChange={() => setSelectedIds((current) => current.includes(item.id) ? current.filter((id) => id !== item.id) : [...current, item.id])} /></td>
      ...
      {view === 'settings' ? (
        <section className="kizc-settings">
          <header><KeyRound size={23} /><div><h3>API Честного знака</h3><p>Токен хранится в WMS зашифрованно и никогда не возвращается в браузер. Закрытый ключ УКЭП сюда не загружается.</p></div></header>
          <div className="kizc-settings-sections">
            <section className="kizc-settings-section kizc-settings-section--token" aria-labelledby="kizc-api-token-title">
              <header>
                <div>
                  <h4 id="kizc-api-token-title">Токен API Честного знака</h4>
                  <p>Вставьте токен True API, полученный в личном кабинете ЧЗ. При следующем открытии поле снова будет пустым — это нормально.</p>
                </div>
                <span className={overview?.connection?.tokenConfigured ? 'is-ready' : 'is-missing'}>
                  {overview?.connection?.tokenConfigured ? 'Токен сохранён' : 'Токен не задан'}
                </span>
              </header>
              <div className="kizc-settings-grid">
                <label className="wide"><span>Контур API</span><select value={connectionForm.apiBaseUrl} onChange={(event) => setConnectionForm({ ...connectionForm, apiBaseUrl: event.target.value })}><option value="https://markirovka.crpt.ru/api/v3/true-api">Промышленный · markirovka.crpt.ru</option><option value="https://markirovka.sandbox.crptech.ru/api/v3/true-api">Тестовый · sandbox.crptech.ru</option></select></label>
                <label className="wide"><span>Токен True API</span><textarea className="token" autoComplete="off" spellCheck={false} value={connectionForm.apiToken} onChange={(event) => setConnectionForm({ ...connectionForm, apiToken: event.target.value })} placeholder={overview?.connection?.tokenConfigured ? 'Оставьте пустым, если токен менять не нужно' : 'Вставьте токен API Честного знака'} /></label>
                <label className="wide"><span>Срок действия токена</span><input type="date" value={connectionForm.tokenExpiresAt} onChange={(event) => setConnectionForm({ ...connectionForm, tokenExpiresAt: event.target.value })} /></label>
              </div>
            </section>
            <section className="kizc-settings-section" aria-labelledby="kizc-owner-title">
              <header><div><h4 id="kizc-owner-title">Организация и сертификат</h4><p>Реквизиты владельца кодов маркировки и данные сертификата для контроля подписи.</p></div></header>
              <div className="kizc-settings-grid">
                ...
                <label><span>Владелец сертификата</span><input value={connectionForm.certificateSubject} onChange={(event) => setConnectionForm({ ...connectionForm, certificateSubject: event.target.value })} /></label>
              </div>
            </section>
          </div>
