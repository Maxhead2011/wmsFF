...
export function StorageZonesPanel({ session }: { session: AuthSession }) {
  ...
  async function deletePallet(pallet: StorageLayout['pallets'][number]) {
    ...
    await run(
      `delete-pallet:${pallet.id}`,
      () => deleteStoragePallet(session.accessToken, pallet.id),
      `Паллета ${pallet.code} удалена.`,
    );
    setExpanded((current) => {
  ...
  return (
    <div className="storage-zones">
      ...
      <section className="storage-zones__toolbar">
        ...
        {data?.warehouse.code === 'MSK' ? (
          <button
            className="secondary-button"
            type="button"
            disabled={!data || busy === 'sync'}
            onClick={() =>
              void run(
                'sync',
                () => syncStorageLayout(session.accessToken, data?.warehouse.id),
                'Размещение ИП Лукина обновлено из Google-таблицы.',
              )
            }
          >
            <RefreshCw size={16} className={busy === 'sync' ? 'spin' : ''} />
            Синхронизировать
          </button>
        ) : null}
        {data?.warehouse.code === 'MSK' && data.googleSync.sourceUrl ? (
          <a className="storage-zones__sheet-link" href={data.googleSync.sourceUrl.replace('/export?format=csv&gid=0', '/edit?gid=0')} target="_blank" rel="noreferrer">
            Открыть таблицу <ExternalLink size={14} />
          </a>
        ) : null}
      </section>
      ...
      {data ? (
        <section className="storage-zones__list">
          ...
          {visiblePallets.map((pallet) => {
            ...
            return (
              <article key={pallet.id} className="storage-pallet">
                ...
                {isOpen ? (
                  <div className="storage-pallet__body">
                    ...
                    <div className="storage-pallet__pallet-actions">
                      <button
