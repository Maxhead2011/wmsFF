...
import { recordSpaceWorkspaceUse } from './lib/spaceNavigation';
...
const MonitoringPanel = lazy(() =>
  import('./components/monitoring/MonitoringPanel').then((module) => ({ default: module.MonitoringPanel })),
);
const DbsPanel = lazy(() =>
...
export function App() {
  ...
  // FIX: Space learns from real navigation without changing server-side permissions or data.
  useEffect(() => {
    if (uiTheme === 'space' && session?.user.id) {
      recordSpaceWorkspaceUse(session.user.id, activeWorkspaceId);
    }
  }, [activeWorkspaceId, session?.user.id, uiTheme]);
...
function renderWorkspace(
  ...
  switch (activeWorkspaceId) {
    ...
    case 'monitoring':
      return <Suspense fallback={<div className="workspace-loading">Подключаю мониторинг…</div>}><MonitoringPanel session={session} /></Suspense>;
    case 'dbs':
  ...
  return (
    <div className="modern-dashboard">
      ...
      <div className="modern-dashboard__columns">
        ...
        {theme !== 'space' ? <section className="modern-dashboard__panel">
