...
export type KizSearchFilter={clientId:string;warehouseId:string;dateFrom?:string;dateTo?:string;requestNumber?:string;supplyId?:string};
...
const query=(filter:KizSearchFilter)=>new URLSearchParams(Object.entries(filter).filter((entry):entry is [string,string]=>entry[1]!==undefined)).toString();
