...
describe('operational notification destinations', () => {
 // TEST: keep the server's existing WB health notification flow.
 it('keeps WB health in monitoring', () => expect(resolveAdminNotificationTarget({...item,type:'WB_SYNC_HEALTH'})).toEqual({workspace:'monitoring'}));
