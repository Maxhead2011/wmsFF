...
export function describeStockTransfer(result: RoutedFbsStockTransferResult): string {
  const messages = result.transfers.map(run => run.status === 'CREATED'
    ? `Перенесено ${run.orderCount}: «${run.supplyName}» (${run.supplyId}), заявка №${String(run.requestNumber).padStart(6, '0')}.${run.portal?.stickers.length ? ' WB выдал новые стикеры: распечатайте их в журнале повторной отгрузки и замените прежние.' : ''}${run.errorMessage ? ` ${run.errorMessage}. Продолжите сверку в журнале повторной отгрузки.` : ''}`
