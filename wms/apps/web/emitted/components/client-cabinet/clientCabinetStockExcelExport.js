...
export function downloadClientCabinetStockExcel(client, stock, canSeeStoragePlaces, activeRequests = []) {
...
function aggregateStockRows(stock, activeRequests) {
...
function applyActiveRequestReservations(rows, requests) {
    ...
    requests
        ...
        if (item.skuId) {
            for (const row of orderedRows.filter((candidate) => candidate.skuIds.has(item.skuId))) {
...
function deductFromRow(row, quantity) {
...
function stockExcelFileName(clientCode) {
