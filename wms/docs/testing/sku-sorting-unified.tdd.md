...
## Дополнение: пилот Сони / №633

По выбору Константина подготовлен пилот для точного device ID и request ID.
Добавлены `sku-sorting-policy.ts`, `sku-sorting-policy.spec.ts`; capability и серверные
start/check/move/receive проверяют эту пару. Общий флаг остаётся false, создание остальных
заявок не меняется. Старый receive тоже не обходит ограничение уже преобразованной заявки.
RED: отсутствующая policy; затем старый receive разрешал чужому устройству обращение
к преобразованной заявке. GREEN: 818/818 API, TypeScript успешно; Android по 35/35.
Изменён только LOGOFF versionCode 155 / versionName `0.1.156-sku-pilot`.
Подписанный release APK собран и apksigner verify пройден; сертификат совпадает с установленной
линейкой LOGOFF. Общие файлы logoff-tsd.apk/json не опубликованы и не заменены.

Предпубликационная read-only проверка 05.09.2026 выявила препятствие в данных №633:

| Исходный короб в маршруте | План без отбора | История переноса 04.09.2026 |
| --- | --- | --- |
| FFL_LKB1007_196 | 2 | В FFL__LKB0409_3: AVAILABLE +2 и RESERVED +2 |
| FFL_LKB1107_176 | 4 | В FFL__LKB0409_3: AVAILABLE +4 и RESERVED +4 |
| FFL_LKVOZ2208_06 | 1 | В FFL__LKB0409_3: AVAILABLE +1 и RESERVED +1 |

В исходных коробах текущих балансов нет, но источник маршрута не обновлён. Эти записи
показывают риск двойного учёта после прежней актуализации, а не подтверждённые физические
14 единиц. Нельзя просто освободить резерв и увеличить доступный остаток.
Автоматический start правильно отклонит такой снимок. Публикация/включение пилота остановлены
до отдельной сверки и решения по этим данным. Production не изменялся, команды обновления
устройству не отправлялись. Следующий шаг — согласованная проверка и исправление данных,
не обход защиты и не массовое снятие резервов.

### Подтверждение факта и адресная коррекция

Константин подтвердил: `FFL__LKB0409_3` — 7 штук SKU заявки №633.
Добавлен отдельный `src/scripts/repair-sku633-pilot.ts`: по умолчанию только анализ,
применение по свежему SHA-256 снимка, serializable-транзакция и idempotent audit ID.
Он сохраняет доступные 7, убирает дублирующие RESERVED 7 с INVENTORY_ADJUSTMENT −7,
освобождает только доказанный собственный резерв 31, переносит маршрут трёх старых источников
в фактический короб, сохраняет 17 PICKED и переключает только №633 в новый режим.
Другие SKU и SHIPPING/PACKING не меняются. Старые источники сохраняются с нулевым планом,
а не удаляются. Любое изменение контрольного снимка останавливает применение.

Для 7 физических единиц обнаружены только 4 зарегистрированных КИЗа. `candidate/move`
теперь допускают регистрацию отсутствующего КИЗ только по существующей записи
INVENTORY_KIZ_SCAN текущего раунда актуализации и свободному месту среди КИЗов доступного
количества. Дополнительного прихода нет. Уже зарегистрированный чужой/несвободный КИЗ
не перепривязывается. Проверка истории отгрузок сохраняется.

TDD: 9 тестов адресной проверки и тест отсутствующего КИЗа; RED воспроизведён до реализации.
GREEN: API 828/828, TypeScript build успешно. Покрытие и реальное ручное прохождение ТСД
по-прежнему не подтверждены автоматическими тестами. Скрипт и его тест — единственные
новые файлы этого шага; дополнительно изменены sku-sorting.service.ts и его тест.
