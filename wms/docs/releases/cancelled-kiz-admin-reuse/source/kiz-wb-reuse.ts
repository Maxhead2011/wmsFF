import { cancelledKizReviewEnabled, cancelledWithoutSale } from './kiz-cancelled-reuse';
...
import { manualWriteoffRecovery, canReuseAfterManualWriteoff, type ManualRecovery } from './kiz-manual-writeoff';
...
export function decideKizReuse(orders: OrderEvidence[], circulation: string | null): ReuseDecision {
  ...
  if (orders.some(o => o.verified && o.containsKiz &&
      (o.wbStatus === 'sold' || (o.supplyAccepted === true && cancelled.has(o.wbStatus ?? '') && !(cancelledKizReviewEnabled() && o.supplierStatus === 'cancel' && cancelledWithoutSale(o.wbStatus)))))) return 'RELABEL';
...
export async function inspectKizReuse(db: PrismaService, clientId: string, kiz: string, excludedId = '') {
  ...
  if (!serial) return { decision: 'REVIEW' as ReuseDecision, checkedAt, history: [], orders: [], circulation: null, manualWriteoffRecovery: null as ManualRecovery|null };
  ...
  if (!history.length && !truncated) return {decision:'ALLOW' as ReuseDecision,checkedAt,history:detailed,orders:[],circulation:null,manualWriteoffRecovery:null as ManualRecovery|null};
  ...
  let recovery:ManualRecovery|null=null;
  if(decision==='REVIEW'&&process.env.WMS_KIZ_REVIEW_QUEUE_ENABLED==='true'&&!truncated&&orderIds.length<=5){
    const proof=await manualWriteoffRecovery(db,clientId,kiz);
    const inactive=tasks.filter(exact).every(t=>t.status==='COMPLETED'&&requests.some(r=>r.id===t.requestId&&['DONE','CANCELLED','REJECTED'].includes(r.status)));
    if(inactive&&canReuseAfterManualWriteoff(decision,proof,orders,history)){decision='ALLOW';recovery=proof;}
  }
  return {decision:decision as ReuseDecision,checkedAt,history:detailed,orders,circulation,manualWriteoffRecovery:recovery};
}
