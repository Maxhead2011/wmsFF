...
Флаг `WMS_WB_STOCK_FINE_SETTINGS=true` включает на нашей ВМС резерв клиента (`marketplace-stock-control.service.ts`, `wb-stock-reserve.ts`), endpoint `PUT administration/marketplace-stock-control/:clientId/reserve` и анализ `wb-stock-demand-analysis.ts`. При выключенном флаге прежние расчёты сохраняются. Резерв применяется в расчётах публикаций и сверки WB, но не меняет StockBalance/ProductMark. Настройка хранится в SystemSetting, отдельно от общего разрешения отправки.

Веб: `AdministrationMarketplaceStockControl.tsx` — резерв; `FbsStockAllocationView.tsx` — ABC по объёму заказанных единиц, XYZ по вариации дневного спроса, рекомендации общих долей и предпросмотр по введённым долям. Рекомендация сама ничего не сохраняет; ручные доли остаются при недостаточной истории. Периоды отсутствия товара на WB не восстановлены, предупреждение показано в интерфейсе. При выключенной общей выгрузке доли можно сохранить без отправки WB. Исторические настройки клиентов автоматически не изменяются.

Тесты: `apps/api/test/wb-stock-{reserve,reserve-settings,reserve-publication,demand-analysis}.spec.ts`, `apps/web/src/lib/wbStockPreview.spec.ts`. API-схема БД не меняется. В проданной ВМС флаг не включать без отдельного согласования.
