...
Фильтр списка FBS по выбранному филиалу: `MarketplaceConnectionsService.scopeFbsOrdersForUser`, тест `apps/api/test/fbs-selected-branch-filter.spec.ts`. Включается серверной переменной `WMS_FBS_SELECTED_BRANCH_FILTER=true`; по умолчанию остаётся прежнее поведение. При включении даже глобальные роли видят заказы по выбранному филиалу: WB — по маршруту склада (`BRANCH`, `CENTRAL` или настроенная автопривязка), Ozon — по филиалу исполнения подключения. Историческое размещение заявки не переопределяет маршрут. В переключателе филиала на экране FBS пункт «Показать всё» передаёт `allBranches=1` в списки заказов и активных клиентов: глобальные роли видят общий список без исключённых складов, ограниченные пользователи — только разрешённые филиалы. Рабочий филиал в профиле не сбрасывается. `App.tsx` пересоздаёт экран FBS при смене режима или филиала, сбрасывая прежний выбор заказов. Переменная не меняет права пользователя и не переносит заказы.
