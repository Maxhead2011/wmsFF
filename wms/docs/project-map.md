...
Сортировка без КИЗ: `inventory/pallet-sorting.service.ts` сохраняет `scanMode` в сессии; отсутствие поля означает прежний `BARCODE_KIZ`. Новый `BARCODE_ONLY` требует `WMS_PALLET_SORTING_BARCODE_ONLY=true` только на нашей ВМС. `stock/stock-operations.service.ts::transferSortingBarcodeUnit` переносит ровно одну AVAILABLE-единицу между подтверждённым исходным и целевым коробами одного клиента и филиала. Привязанные КИЗ запрещают этот путь; автоматического выбора КИЗ и оприходования недостающего товара нет. Повтор HTTP-запроса сохраняет operationId, следующий физический скан создаёт новый. Выбор режима: web `PalletSortingPanel.tsx`, Android `PalletSortingScreen.java` и `PalletSortingScanMode.java`. Тесты: `pallet-sorting-barcode-only.spec.ts`, `PalletSortingScanModeTest.java`. При выпуске Android требуется перенос на актуальную серверную версию с сохранением локализации и других уже опубликованных функций.

Контроль циклов WB: `marketplace-connections/wb-sync-health.{service,controller,rules}.ts`, вкладка `monitoring/WbSyncHealthPanel.tsx`. Флаг `WMS_WB_SYNC_HEALTH_ENABLED=true` включается только на нашей ВМС. Начало и завершение фонового цикла сохраняются в `SystemSetting` под `monitor.wbSync.<clientId>`: последние 20 циклов, часовой срок проверки и одна активная проблема. Подтверждения проверяются по свежему runId и совпадению количеств; ошибки биллинга отделены от заказов. При отсутствии завершения за час создаётся уведомление, при ошибке проверяется следующий завершённый цикл, восстановление возвращает часовой интервал. Наблюдатель не отправляет остатки и не меняет счета. Тесты: `wb-sync-health.spec.ts`, `wb-sync-health-service.spec.ts`. История до включения не восстанавливается; отсутствие свежих доказательств не отображается как успех.

Флаг `WMS_FBS_BILLING_DAILY_TRANSACTIONS=true` изолирует расчёт FBS-биллинга на нашей ВМС: `ensureFbsProcessingCharges` группирует целые поставки по прежнему расчётному дню, выполняет отдельную `runBillingMutation` для каждого дня, сохраняя общую финансовую блокировку и дневной расчёт логистики. Повторный запуск использует прежние sourceKey и проверки оплаченных счетов. В `refreshAllFbsClients` сначала загружаются заказы без расчёта биллинга и обновляются остатки; затем пересчитываются начисления. Сбой биллинга журналируется отдельно, сохранённые суммы в кэше не обнуляются. Без флага остаётся прежний путь. Регрессии: `apps/api/test/fbs-billing-cycle-isolation.spec.ts`.
...
`WbStockPublicationCheck.chrtId` хранится как BIGINT (миграция `20260918110000_wb_stock_chrt_bigint`); `wbStockCheckDto` переводит его в проверенный safe integer для JSON/WB. После миграции нужно пересоздать Prisma Client и перезапустить API. Перед отправкой `selectKnownWbStockTargets` исключает размер из всех складов, если WB не вернул его остаток хотя бы на одном; остальные размеры проходят прежнюю проверку уменьшений и увеличений. Пропуски получают UNCONFIRMED и lastError, не получают успешной даты синхронизации; отсутствие записи не считается нулём. Кнопка проверки возвращает отдельный счётчик unconfirmed.
...
Флаг `WMS_FBS_RELABEL_SOURCE_BOX_GUARD=true` включает на нашей ВМС проверку исходного короба при переклейке FBS. `scanFbsTsdKiz` отклоняет КИЗ из другого короба до изменения SKU и обращения к WB; `moveExistingFbsKizToOpenedBox` повторяет проверку в транзакции. Подтверждение переноса не обходит запрет. Изменение SKU использует проверку прежних boxId/skuId/status. Обычный отбор и замена использованного КИЗа сохраняют прежний путь. Флаг по умолчанию выключен для совместимости проданной ВМС. Это защита от перепривязки, а не перенос всей переклейки на этап КИЗа: ранее выполненное подтверждение нового ШК не откатывается. Регрессии: `apps/api/test/fbs-relabel-source-box.spec.ts`.
